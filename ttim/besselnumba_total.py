...
def lapls_int_ho(x, y, z1, z2, order):
    ...
    for i in range(1, order + 2):
        zpower = zpower * z
        omega[i - 1] = (
            -L / (4 * np.pi * i) * (zpower * log1 + qm[i] - log2 + (-1) ** i * log3)
        )
...
def bessellsreal(x, y, x1, y1, x2, y2, lab):
    ...
    for n in range(41):
        ...
        omega = omega + (
            2 * alpha[n] * log2 - 2 * alpha[n] / (n + 1) + beta[n]
        ) * term2 / (n + 1)
        omega = omega - (
            2 * alpha[n] * log1 - 2 * alpha[n] / (n + 1) + beta[n]
        ) * term1 / (n + 1)
...
def bessellsrealho(x, y, x1, y1, x2, y2, lab, order):
    ...
    for p in range(order + 1):
        ...
        for n in range(40 + p + 1):
            ...
            term2 = term2 * d2minzeta
            omega = omega + (
                2 * alphanew[n] * log2 - 2 * alphanew[n] / (n + 1) + betanew[n]
            ) * term2 / (n + 1)
            omega = omega - (
                2 * alphanew[n] * log1 - 2 * alphanew[n] / (n + 1) + betanew[n]
            ) * term1 / (n + 1)
...
def bessellsv(x, y, z1, z2, lab, order, R, nlab):
    ...
    for n in range(0, nlab):
        omega[n * nterms : (n + 1) * nterms] = bessells(
            x, y, z1, z2, lab[n], order, d1, d2
        )
    return omega
...
def bessellsqxqyv(x, y, z1, z2, lab, order, R, nlab):
    ...
    for n in range(nlab):
        ...
        qxqy[(n) * nterms + nhalf : (n + 1) * nterms + nhalf] = qxqylab[
            order + 1 : 2 * order + 1 + 1
        ]
    return qxqy
...
def besselldv(x, y, z1, z2, lab, order, R, nlab):
    ...
    d1, d2 = find_d1d2(z1, z2, complex(x, y), R * np.abs(lab[0]))
    for n in range(nlab):
        omega[(n) * nterms : (n + 1) * nterms] = besselld(
            x, y, z1, z2, lab[n], order, d1, d2
        )
    return omega
...
def bessells_circcheck(x, y, z1in, z2in, lab):
    ...
    Lzero = 20
    z = complex(x, y)
    x1, y1, x2, y2, Npt = circle_line_intersection(z1in, z2in, z, Lzero * np.abs(lab))
